...
export default function ProjectCard({ title, description, liveUrl = "", githubUrl, highlight = false, slug }) {
  const cardClasses = `group relative p-4 sm:p-6 border border-border hover:border-primary/40 hover:bg-accent/30 transition-all duration-300 -mx-4 sm:-mx-6 flex flex-col justify-between ${!highlight ? 'h-56' : ''}`;

  return (
    <div className={cardClasses}>
      <a href={`/projects/${slug}`} className="absolute inset-0 z-10" aria-label={`View project: ${title}`}></a>
      <div className="space-y-3">
        <h3 className="font-bold tracking-tight text-base uppercase group-hover:text-primary transition-colors duration-300">{title}</h3>
      ...
      <div className="flex justify-between items-center pt-4 z-20 relative">
        <div className="flex gap-3">
          {liveUrl && (
            <Button variant="transparent" asChild className="gap-2 hover:text-primary transition-all duration-300">
